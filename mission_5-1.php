...
<?php
    ...
    if(isset($_POST["Ssubmit"])&&empty($_POST["longnumber"])&&!empty($_POST["pass"]))
    {
        ...
        $sql = $pdo -> prepare("INSERT INTO pop (name,comment,datetime,password) VALUES (:name, :comment, :datetime, :password)");
    ...
    if(isset($_POST["Ssubmit"])&&isset($_POST["longnumber"]) && !empty($_POST["pass"]))
    {
	    ...
	    $sql = 'UPDATE pop SET name=:name,comment=:comment WHERE id=:id';
	    $stmt = $pdo->prepare($sql);
